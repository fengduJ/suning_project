	...
	<div class="rapper">
		...
		<div class="property" style="margin-top: 15px;">
			...
			<table class="tables" border="1" cellspacing="0">
				<tr>
					<td>延保单单号</td>
					<td><input type="text" name="orderNoSource" class="orderNo" :value="orderNo" readonly="readonly"></td>
					<td>经销商</td>
				...
				<tr>
					...
					<td>订单号</td>
					<td><input type="text" name="orderNo" class="orderNoSource" v-model="orderNoSource" placeholder="请输入" required></td>
				</tr>
			</table>
		</div>
		<div class="submitBtn">
...
//	import MyAjax from "../../../assets/MyAjax.js";
	...
	export default {
		...
		mounted() {
			// 进入详情页，判断有无 "订单号"
               if(this.$store.state.selectOrderno ==''){
               	    this.state = 'add';
               }else{
                 	this.state = 'upd';
					this.orderNo = this.$store.state.selectOrderno
		...
		methods: {
			...
			back() {
				...
				this.$store.state.selectOrderno =''
			},
			submit() {
